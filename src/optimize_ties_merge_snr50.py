...
def optimize_ties_params(
    ...
    """
    Optimize TIES weights and densities with Optuna.
    template_config_path: path of YAML configuration to update
    """
    ...
    save_dir = "./experiments/results/optuna_ties_snr50"

    def objective(trial):
        w_sst2 = 1.0 

        w_agnews_low = trial.suggest_float("w_agnews_low", 0.3, 0.9)
        w_mnli_low = trial.suggest_float("w_mnli_low", 0.3, 0.9)
        d_sst2_low = trial.suggest_float("d_sst2_low", 0.15, 0.45)
        d_agnews_low = trial.suggest_float("d_agnews_low", 0.2, 0.45)
        d_mnli_low = trial.suggest_float("d_mnli_low", 0.35, 0.65)
        ...
        print(enumerate(current_config['slices']))
            ...
            for source in sources:
                ...
                if "sst2" in model_path:
                    ...
                    if i==4:
                      ...
                      source['parameters']['density'] = d_sst2_low
                    ...
                    if i==4:
                      ...
                      source['parameters']['weights'] = w_agnews_low
                      source['parameters']['density'] = d_agnews_low
                elif "mnli" in model_path:
                    if i==4:
                      source['parameters']['weight'] =w_mnli_high
                      source['parameters']['density'] = d_mnli_high
                    else:
                      source['parameters']['weight'] =  w_mnli_low
                      source['parameters']['density'] =  d_mnli_low
                elif "base" in model_path:
                    if i == 0 or i==2 or i==4:
                        source['parameters']['weight'] = 0.00
                    else: 
                        source['parameters']['weight'] = 1.00
                        source['parameters']['density'] = 1.00
        ...
        try:
            ...
            print(f"[Trial {trial_id} Result] w_AG_low={w_agnews_low:.2f}, w_MNLI_low={w_mnli_low:.2f}, w_AG_high={w_agnews_high:.2f}, w_MNLI_high={w_mnli_high:.2f}")
            print(f" d_AG_low={d_agnews_low:.2f}, d_MNLI_low={d_mnli_low:.2f}, d_AG_high={d_agnews_high:.2f}, d_MNLI_high={d_mnli_high:.2f} |")
            print(f" d_sst2_low={d_sst2_low:.2f}, d_sst2_high={d_sst2_high:.2f}||||| Score: {score:.4f}")
            return score
